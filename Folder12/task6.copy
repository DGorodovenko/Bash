...
if [ $# -ne 1 ] ; then
	echo "Skrypt musi byc wywolany z argumentem w postaci wielkosci boku 
kwadratu!"
...
#jak jest wartosc boku
	...
	#jezeli warunki sa spelnione
		...
		for (( y=1 ; $y <= $1 ; y++ )) ; do
			...
			#kiedy y jest rowny pierwszej i ostatniej wartosci z 
ustalonego zakresu (rysowanie górnej i dolnej krawedzi)
		...
		#rysowanie lewej i prawej krawedzi
			...
			#tworzymy gwiazdki dla lewej i prawej krawedzi w jednej 
linii
			echo -n "*"
			#rysowanie spacji od drugiej wartosci do momentu aż nie 
zrównamy sie pozycja z ostatnia wartoscia
			for (( z=2 ; $z < $1 ; z++ )) ; do
				echo -n " "
			done
			#na tym etapie wciaz wszsystko jest w jednej linicje 

			#wypisz gwiazdki - to sprawia ze wyrysowane linie 
przestawiaja sie na odpowiednie miejsce
			echo "*"
		fi
	done
	fi
